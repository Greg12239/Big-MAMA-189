...
function initializeHeroPhotoStack() {
  const stack = document.querySelector("[data-hero-photo-stack]");
  if (!stack) return;

  const photos = Array.from(stack.querySelectorAll(".hero-photo"));
  const suppliedPhoto = stack.querySelector('[data-photo-id="street-tattoo-burger"] img');
  const expectedSlots = ["primary", "secondary", "rear"];
  const slotTransitionDuration = 480;
  const slotTransitionFallback = slotTransitionDuration + 160;
  ...
  if (photos.length !== 3 || !suppliedPhoto) return;

  const hasValidDeckState = () => {
    const slots = photos.map((photo) => photo.dataset.slot);
    const selectedCount = photos.filter((photo) => photo.getAttribute("aria-pressed") === "true").length;

    return expectedSlots.every((slot) => slots.filter((currentSlot) => currentSlot === slot).length === 1) && selectedCount === 1;
  };

  const setFocusedPhoto = (selectedPhoto) => {
    photos.forEach((photo) => {
      photo.setAttribute("aria-pressed", String(photo === selectedPhoto));
    });
  };

  const getFocusedForegroundPhoto = () => {
    const focusedPhoto = photos.find((photo) => photo.getAttribute("aria-pressed") === "true");
    if (focusedPhoto && ["primary", "secondary"].includes(focusedPhoto.dataset.slot)) return focusedPhoto;

    return photos.find((photo) => photo.dataset.slot === "primary") || null;
  };

  const clearTransactionEffects = (transaction) => {
    window.cancelAnimationFrame(transaction.startFrame);
    window.clearTimeout(transaction.fallbackTimer);
    transaction.promotedPhoto.removeEventListener("transitionend", transaction.handleTransformEnd);
    transaction.demotedPhoto.removeEventListener("transitionend", transaction.handleTransformEnd);
    stack.classList.remove("is-transitioning");
    photos.forEach((photo) => photo.removeAttribute("data-motion"));
  };

  const finishTransaction = (transaction) => {
    if (activeTransaction !== transaction || transaction.generation !== transactionGeneration) return;

    clearTransactionEffects(transaction);
    activeTransaction = null;

    if (!hasValidDeckState()) {
      queuedPhoto = null;
      return;
    }

    const pendingPhoto = queuedPhoto;
    queuedPhoto = null;

    if (!pendingPhoto || pendingPhoto.getAttribute("aria-pressed") === "true" || pendingPhoto.disabled) return;

    window.requestAnimationFrame(() => {
      if (interactionEnabled && hasValidDeckState()) activatePhoto(pendingPhoto);
    });
  };

  const beginSwap = (promotedPhoto, demotedPhoto) => {
    if (!hasValidDeckState()) return;
    ...
    const transaction = {
      ...
      promotedPhoto,
      demotedPhoto,
      completedPhotos: new Set(),
      fallbackTimer: 0,
      handleTransformEnd: null,
      startFrame: 0,
      travelling: false
    };
    ...
    promotedPhoto.dataset.motion = "promoting";
    demotedPhoto.dataset.motion = "demoting";

    const completeTransform = (event) => {
      if (
        activeTransaction !== transaction ||
        transaction.generation !== transactionGeneration ||
        !transaction.travelling ||
      ...
      transaction.completedPhotos.add(event.currentTarget);
      if (transaction.completedPhotos.size === 2) finishTransaction(transaction);
    };

    transaction.handleTransformEnd = completeTransform;

    const applyLogicalSwap = () => {
      const previousForegroundSlot = demotedPhoto.dataset.slot;
      demotedPhoto.dataset.slot = "rear";
      promotedPhoto.dataset.slot = previousForegroundSlot;
      setFocusedPhoto(promotedPhoto);
    };
    ...
    transaction.startFrame = window.requestAnimationFrame(() => {
      if (activeTransaction !== transaction || transaction.generation !== transactionGeneration) return;

      transaction.travelling = true;
      promotedPhoto.addEventListener("transitionend", completeTransform);
      demotedPhoto.addEventListener("transitionend", completeTransform);
      transaction.fallbackTimer = window.setTimeout(() => finishTransaction(transaction), slotTransitionFallback);
      applyLogicalSwap();
    });
  };

  const activatePhoto = (selectedPhoto) => {
    if (!interactionEnabled || selectedPhoto.disabled || !hasValidDeckState()) return;

    if (activeTransaction) {
      queuedPhoto = selectedPhoto;
      return;
    }

    if (selectedPhoto.dataset.slot !== "rear") {
      setFocusedPhoto(selectedPhoto);
      return;
    }

    const focusedForegroundPhoto = getFocusedForegroundPhoto();
    if (!focusedForegroundPhoto) return;

    beginSwap(selectedPhoto, focusedForegroundPhoto);
  };

  stack.addEventListener("click", (event) => {
    if (!(event.target instanceof Element)) return;

    const selectedPhoto = event.target.closest(".hero-photo");
    if (!(selectedPhoto instanceof HTMLButtonElement) || !stack.contains(selectedPhoto)) return;

    activatePhoto(selectedPhoto);
  });

  const enableInteraction = () => {
    if (interactionEnabled || suppliedPhoto.naturalWidth === 0) return;
  ...
  const decodeSuppliedPhoto = () => {
    if (suppliedPhoto.naturalWidth === 0) return;

    readinessTimer = window.setTimeout(enableInteraction, 1200);

    if (typeof suppliedPhoto.decode !== "function") {
      enableInteraction();
      return;
    }

    suppliedPhoto.decode().catch(() => undefined).finally(enableInteraction);
  };

  if (suppliedPhoto.complete) {
    decodeSuppliedPhoto();
  } else {
    suppliedPhoto.addEventListener("load", decodeSuppliedPhoto, { once: true });
  }
}

initializeHeroPhotoStack();
...
class MenuCategoryController {
  ...
  init() {
    ...
    menuCategorySwitcher?.addEventListener("pointerdown", () => this.cancelCategoryRailMotion(), { passive: true });
    menuCategorySwitcher?.addEventListener("touchstart", () => this.cancelCategoryRailMotion(), { passive: true });
    menuCategorySwitcher?.addEventListener("wheel", () => this.cancelCategoryRailMotion(), { passive: true });
  ...
  cancelCategoryRailMotion() {
    if (!this.railMotion) return;
    window.cancelAnimationFrame(this.railMotion.frame);
    this.railMotion = null;
  }
  ...
  moveCategoryRail(tab, direction = "neutral", { immediate = false } = {}) {
    ...
    if (immediate || prefersReducedMotion || Math.abs(target - start) <= 1) {
      menuCategorySwitcher.scrollLeft = target;
      return;
    }

    const motion = { frame: 0, startedAt: 0, lastWriteAt: 0 };
    const step = (timestamp) => {
      if (this.railMotion !== motion) return;
      if (!motion.startedAt) motion.startedAt = timestamp;
      const progress = Math.min(1, (timestamp - motion.startedAt) / menuRailTravelDuration);
      motion.lastWriteAt = performance.now();
      menuCategorySwitcher.scrollLeft = start + (target - start) * cubicBezierEase(progress);
      if (progress < 1) motion.frame = window.requestAnimationFrame(step);
      else this.railMotion = null;
    };
  ...
  handleCategoryRailScroll() {
    ...
    if (motion && performance.now() - motion.lastWriteAt > 48) this.cancelCategoryRailMotion();
    this.closeWhenBurgerLeavesRail();
  }
  ...
  selectTopCategory(key) {
    ...
    this.topPreparation = { key, request };
    this.waitForPanelImages(nextPanel, { prewarm: true }).finally(() => {
      ...
      this.startTopCategoryTransition(key, nextPanel, request);
    });
  }

  startTopCategoryTransition(key, nextPanel, generation) {
    ...
    this.moveCategoryRail(this.tabs[nextIndex], railDirection);
