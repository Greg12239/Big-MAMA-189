(() => {
  ...
  class ReviewOrbitController {
    ...
    syncStateAttributes() {
      ...
      this.deck.dataset.reviewPendingTargetStep = this.pendingTargetStep
        ? String(this.pendingTargetStep.step)
        : "";
    ...
    requestDelta(delta, source = "manual") {
      if (!delta) return;
      const reference = this.pendingTargetStep?.step
        ?? this.currentTargetStep
        ?? this.settledStep;
      this.requestStep(reference + delta, source);
    }
    ...
    requestTarget(index, source = "manual") {
      const reference = this.pendingTargetStep?.step
        ?? this.currentTargetStep
        ?? this.settledStep;
      this.requestStep(this.resolveTargetStep(index, reference), source);
    }
    ...
    requestStep(targetStep, source = "manual") {
      ...
      if (this.phase !== "idle") {
        this.pendingTargetStep = { step: target, source };
        this.syncStateAttributes();
        return;
      }
    ...
    commitImmediate(step) {
      ...
      this.syncVisualStates();
      this.syncContentStates();
      if (this.getMode() === "mobile") this.applyAllPoses(step, this.metrics(), false);
      this.announceActive();
      this.syncStateAttributes();
    }
    ...
    durationFor(distance) {
      if (distance >= 2.5) return 940;
      if (distance >= 1.5) return 860;
      if (distance >= 0.75) return 680;
      return 440;
    }
    ...
    async startSettle(targetStep, source = "manual") {
      if (this.phase !== "idle") {
        this.pendingTargetStep = { step: Math.round(targetStep), source };
        return;
      }
      ...
      if (distance < 0.001 || this.getMode() !== "mobile") {
        this.commitImmediate(target);
        return;
      }

      const metrics = this.metrics();
      const duration = this.durationFor(distance);
      ...
      try {
        ...
        const pending = this.pendingTargetStep;
        this.pendingTargetStep = null;
        if (pending && pending.step !== this.settledStep) {
          this.startSettle(pending.step, pending.source);
        } else {
          this.syncStateAttributes();
        }
      } catch {
    ...
    cancelAnimations(restore = false) {
      ...
      this.releaseAnimations();
      this.currentTargetStep = null;
      if (restore && this.getMode() === "mobile" && this.slots) {
    ...
    handlePointerDown(event) {
      if (this.getMode() !== "mobile" || this.phase !== "idle") return;
    ...
    handlePointerMove(event) {
      ...
      const resistedProgress = magnitude > 1.2
        ? Math.sign(rawProgress) * (1.2 + (magnitude - 1.2) * 0.18)
        : rawProgress;

      pointer.rawProgress = rawProgress;
    ...
    handlePointerEnd(event, cancelled = false) {
      ...
      if (shouldCommit) {
        const direction = Math.sign(pointer.rawProgress || -pointer.velocityX) || 1;
        const isLongGesture = Math.abs(pointer.rawProgress) >= 1.45
          || (distance >= pointer.cachedSceneWidth * 0.58 && velocity >= 0.6);
        targetStep += direction * (isLongGesture ? 2 : 1);
      }
